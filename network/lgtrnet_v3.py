...
class LgattBlock(nn.Module):
    expansion = 1
    def __init__(self, in_channels, out_channels, stride=1):
        super(LgattBlock, self).__init__()
        #local-global block
        self.conv_op = nn.Sequential(
                nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1),
                nn.BatchNorm1d(out_channels),
                nn.ReLU(inplace=True),
                nn.Conv1d(in_channels=out_channels, out_channels=out_channels * LgattBlock.expansion, kernel_size=3, padding=1),
                nn.BatchNorm1d(out_channels * LgattBlock.expansion),
                nn.ReLU(inplace=True)
        )
        self.channel_op = nn.Sequential(
                nn.Conv1d(in_channels=in_channels, out_channels=out_channels * LgattBlock.expansion, kernel_size=1),
                nn.BatchNorm1d(out_channels * LgattBlock.expansion),
                nn.ReLU(inplace=True)
        )
        self.fc_att = nn.Linear(3*in_channels, in_channels)

    def att_cal(self, x, att):
        att_scores = F.softmax(att, dim=1)
        output = x.mul(att_scores)
    ...
    def forward(self, x):
        ...
        local_att = self.conv_op(x.permute(0, 2, 1)).permute(0, 2, 1)
        
        current_x = x[:, -1, :]
        global_x = self.att_cal(x, global_att)
        local_x = self.att_cal(x, local_att)
       
        output = torch.cat((current_x, global_x, local_x), 1) 
        # print(output.shape)
        # input('check')
        output = self.fc_att(output)

        return output

class LgattAdpBlock(nn.Module):
    expansion = 1
    def __init__(self, in_channels, out_channels, stride=1):
        super(LgattAdpBlock, self).__init__()
        #local-global block
        self.conv_op = nn.Sequential(
                nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1),
                nn.BatchNorm1d(out_channels),
                nn.ReLU(inplace=True),
                nn.Conv1d(in_channels=out_channels, out_channels=out_channels * LgattAdpBlock.expansion, kernel_size=3, padding=1),
                nn.BatchNorm1d(out_channels * LgattAdpBlock.expansion),
                nn.ReLU(inplace=True)
        )
        self.channel_op = nn.Sequential(
                nn.Conv1d(in_channels=in_channels, out_channels=out_channels * LgattAdpBlock.expansion, kernel_size=1),
                nn.BatchNorm1d(out_channels * LgattAdpBlock.expansion),
                nn.ReLU(inplace=True)
        )
        self.gamma_wt = nn.Sequential(
                nn.AdaptiveAvgPool1d(1),
                nn.Conv1d(in_channels, 3, kernel_size=1, bias=False),
                #nn.BatchNorm2d(1),
                nn.Sigmoid()
        )
...
class Model(nn.Module):
    def __init__(self, args, n_items, DEVICE):
        ...
        ###attention block
        self.att_lg = LgattBlock(self.lstm_size, 1, 1)
