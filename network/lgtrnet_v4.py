...
class Model(nn.Module):
    def __init__(self, args, n_items, DEVICE):
        ...
        self.num_block = [1, 2, 2]
        self.lgtr1_x = self._make_layer(LgTrBlock, 300, self.num_block[0], 1)
        # self.lgtr2_x = self._make_layer(LgTrBlock, 100, self.num_block[1], 1)
        # self.lgtr3_x = self._make_layer(LgTrBlock, 200, self.num_block[2], 1)
        #self.lgtr4_x = self._make_layer(LgTrBlock, 512, num_block[3], 2)
        #self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.fc = nn.Linear(300*LgTrBlock.expansion, n_items)
    ...
    def forward(self, x, prev_state):
        ...
        output = self.lgtr1_x(embed)
        # output = self.lgtr2_x(output)
        # output = self.lgtr3_x(output)
        # output = self.avg_pool(output.permute(0, 2, 1))
