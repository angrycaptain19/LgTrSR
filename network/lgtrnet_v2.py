...
class Model(nn.Module):
    def __init__(self, args, n_items, DEVICE):
        ...
        self.fc_1 = nn.Linear(self.lstm_size, int(n_items // 2))
        #self.fc_2 = nn.Linear(self.lstm_size, int(n_items // 3))
        #self.fc_3 = nn.Linear(self.lstm_size, int(n_items // 3))
        self.fc_2 = nn.Linear(self.lstm_size, n_items - int(n_items // 2))

        self.shortcut_1_2 = nn.Sequential(
                nn.Conv1d(in_channels=int(n_items // 2), out_channels=self.lstm_size, kernel_size=1, bias=False),
                nn.BatchNorm1d(self.lstm_size),
                nn.ReLU(inplace=True),
                nn.Conv1d(in_channels=self.lstm_size, out_channels=(n_items - int(n_items // 2)), kernel_size=1, bias=False),
                nn.BatchNorm1d((n_items - int(n_items // 2))),
                nn.ReLU(inplace=True),
        )
        # self.shortcut_2_3 = nn.Sequential(
        #         nn.Conv1d(in_channels=int(n_items // 3), out_channels=self.lstm_size, kernel_size=1, bias=False),
        #         nn.BatchNorm1d(self.lstm_size),
        #         nn.ReLU(inplace=True),
        #         nn.Conv1d(in_channels=self.lstm_size, out_channels=(n_items - int(n_items // 3 * 2)), kernel_size=1, bias=False),
        #         nn.BatchNorm1d((n_items - int(n_items // 3 * 2))),
        #         nn.ReLU(inplace=True),
        # )
        # self.shortcut_3_4 = nn.Sequential(
        #         nn.Conv1d(in_channels=int(n_items // 4), out_channels=self.lstm_size, kernel_size=1, bias=False),
        #         nn.BatchNorm1d(self.lstm_size),
        #         nn.ReLU(inplace=True),
        #         nn.Conv1d(in_channels=self.lstm_size, out_channels=(n_items - int(n_items // 4 * 3)), kernel_size=1, bias=False),
        #         nn.BatchNorm1d((n_items - int(n_items // 4 * 3))),
        #         nn.ReLU(inplace=True),
        # )
    def forward(self, x, prev_state):
        ...
        output_1 = self.fc_1(output)
        output_2 = self.fc_2(output)
        #output_3 = self.fc_3(output)  
        #output_4 = self.fc_4(output)

        output_2 = self.shortcut_1_2(output_1.permute(0, 2, 1)).permute(0, 2, 1) + output_2
        #output_3 = self.shortcut_2_3(output_2.permute(0, 2, 1)).permute(0, 2, 1) + output_3
        #output_4 = self.shortcut_3_4(output_3.permute(0, 2, 1)).permute(0, 2, 1) + output_4

        logits = torch.cat((output_1, output_2), 2)
