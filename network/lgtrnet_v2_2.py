...
class ConvFc(nn.Module):
    def __init__(self, in_channels, out_channels, conv_channels, last_layer=False):
        super(ConvFc, self).__init__()
        self.last_layer = last_layer
        self.fc = nn.Linear(in_channels, out_channels).cuda()
        self.shortcut = nn.Sequential(
                nn.Conv1d(in_channels=out_channels, out_channels=in_channels, kernel_size=1, bias=False),
                nn.BatchNorm1d(in_channels),
                nn.ReLU(inplace=True),
                nn.Conv1d(in_channels=in_channels, out_channels=conv_channels, kernel_size=1, bias=False),
    ...
    def forward(self, x):
        ...
        if not self.last_layer:
            output_conv = self.shortcut(output.permute(0, 2, 1)).permute(0, 2, 1)
...
class Model(nn.Module):
    ...
    ############################   DEFINE   #######################################
        self.CUT_NUM = 4
        self.convfc_list = self._make_layer(self.CUT_NUM, ConvFc)
    def _make_layer(self, num_blocks, block):
        assert num_blocks >= 3, print('the num blocks must be 3 at least!')
        out_channels = int(self.n_items / num_blocks)
        layers = []
        for _ in range(num_blocks-2):
            #print(out_channels)
            layers.append(block(self.lstm_size, out_channels, out_channels, False))
        layers.append(block(self.lstm_size, out_channels, self.n_items - int(out_channels*(num_blocks - 1)), False))
        layers.append(block(self.lstm_size, self.n_items - int(out_channels*(num_blocks - 1)), self.n_items - int(out_channels*(num_blocks - 1)), True))
        return layers
    ####################################################################################
    ...
    ############################    USAGE  ################################
        result = []
        output_x, convfc_x = self.convfc_list[0](output)
        result.append(output_x)
        for i in range(self.CUT_NUM-1):
            prev_convfc = convfc_x
            output_x, convfc_x = self.convfc_list[i+1](output)
            result.append(output_x + prev_convfc)

        logits = torch.cat(result, 2)
        # print([output.shape for output in result])
        # print(logits.shape)
        # input('check')
    #######################################################################
